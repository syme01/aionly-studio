...
const logger = loggerService.withContext('MarketOAuth')

const MARKET_OAUTH_SERVER = 'https://open.cherryin.ai'
const MARKET_TOPUP_URL = 'https://open.cherryin.ai/console/topup'
...
interface MarketOAuthProps {
  providerId: string
}

const MarketOAuth: FC<MarketOAuthProps> = ({ providerId }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const balance = await window.api.market.getBalance(MARKET_OAUTH_SERVER)
  ...
  const handleLogout = useCallback(() => {
    window.modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          await window.api.market.logout(MARKET_OAUTH_SERVER)
  ...
  const handleTopup = useCallback(() => {
    window.open(MARKET_TOPUP_URL, '_blank')
  }, [])
  ...
  return (
    <Container>
      ...
      <ProviderLogo src={MarketProviderLogo} onClick={() => window.open('https://open.cherryin.ai', '_blank')} />
      {renderContent()}
      <Description>
