...
const api = {
  ...
  market: {
    saveToken: (accessToken: string, refreshToken?: string) =>
      ipcRenderer.invoke(IpcChannel.Market_SaveToken, accessToken, refreshToken),
    hasToken: (): Promise<boolean> => ipcRenderer.invoke(IpcChannel.Market_HasToken),
    getBalance: (apiHost: string) => ipcRenderer.invoke(IpcChannel.Market_GetBalance, apiHost),
    logout: (apiHost: string) => ipcRenderer.invoke(IpcChannel.Market_Logout, apiHost),
    startOAuthFlow: (oauthServer: string, apiHost?: string) =>
      ipcRenderer.invoke(IpcChannel.Market_StartOAuthFlow, oauthServer, apiHost),
    exchangeToken: (code: string, state: string) => ipcRenderer.invoke(IpcChannel.Market_ExchangeToken, code, state)
  },
  // Binary related APIs
  ...
  marketapi: {
    generateSignature: (params: { method: string; path: string; query: string; body: Record<string, any> }) =>
      ipcRenderer.invoke(IpcChannel.Market_GetSignature, params)
  },
  windowControls: {
