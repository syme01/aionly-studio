...
function getAppDataPathFromConfig() {
  try {
    ...
    if (isLinux && process.env.APPIMAGE) {
      ...
      // 这样可以确保获取到正确的可执行文件路径
      executablePath = path.join(path.dirname(process.env.APPIMAGE), 'aionly-studio.appimage')
    }

    if (isWin && isPortable) {
      executablePath = path.join(process.env.PORTABLE_EXECUTABLE_DIR || '', 'AiiOnly-portable.exe')
    }
