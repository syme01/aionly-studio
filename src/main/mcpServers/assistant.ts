...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description: 'Navigate AiOnly to a specific page. Refer to the route table in your skills for available paths.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read AiOnly runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
...
class AssistantServer {
  ...
  private async checkUpdate() {
    try {
      ...
      const response = await fetch('https://api.github.com/repos/aionly/studio/releases/latest', {
        method: 'GET',
        headers: { Accept: 'application/vnd.github.v3+json', 'User-Agent': 'AiOnly' },
        signal: controller.signal
      })
      clearTimeout(timeout)
