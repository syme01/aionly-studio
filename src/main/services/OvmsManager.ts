...
class OvmsManager {
  ...
  public async runOvms(): Promise<{ success: boolean; message?: string }> {
    ...
    const ovmsDir = path.join(homeDir, HOME_APP_DIR, 'ovms', 'ovms')
  ...
  public async getOvmsStatus(): Promise<'not-installed' | 'not-running' | 'running'> {
    const homeDir = homedir()
    const ovmsPath = path.join(homeDir, HOME_APP_DIR, 'ovms', 'ovms', 'ovms.exe')
  ...
  public async isNameAndIDAvalid(modelName: string, modelId: string): Promise<boolean> {
    ...
    const homeDir = homedir()
    const configPath = path.join(homeDir, HOME_APP_DIR, 'ovms', 'ovms', 'models', 'config.json')
    try {
      if (!(await fs.pathExists(configPath))) {
  ...
  private async applyModelPath(modelDirPath: string): Promise<boolean> {
    const homeDir = homedir()
    const patchDir = path.join(homeDir, HOME_APP_DIR, 'ovms', 'patch')
  ...
  public async addModel(
    ...
    const homeDir = homedir()
    const ovdndDir = path.join(homeDir, HOME_APP_DIR, 'ovms', 'ovms')
