...
import { MARKET_OAUTH_CONFIG } from '@shared/config/constant'
...
const logger = loggerService.withContext('MarketOAuthService')
...
class MarketOAuthServiceError extends Error {
  constructor(
    message: string,
    public readonly cause?: unknown
  ) {
    super(message)
    this.name = 'MarketOAuthServiceError'
  }
}
...
class MarketOAuthService {
  /**
   * Validate API host against allowlist to prevent SSRF attacks
   */
  private validateApiHost(apiHost: string): void {
    if (!MARKET_OAUTH_CONFIG.ALLOWED_HOSTS.includes(apiHost)) {
      throw new MarketOAuthServiceError(`Unauthorized API host: ${apiHost}`)
    }
  }
  ...
  public startOAuthFlow = async (
    ...
    authUrl.searchParams.set('client_id', MARKET_OAUTH_CONFIG.CLIENT_ID)
    authUrl.searchParams.set('redirect_uri', MARKET_OAUTH_CONFIG.REDIRECT_URI)
    authUrl.searchParams.set('response_type', 'code')
    authUrl.searchParams.set('scope', MARKET_OAUTH_CONFIG.SCOPES)
  ...
  public exchangeToken = async (
    ...
    if (!flowData) {
      throw new MarketOAuthServiceError('OAuth flow expired or not found')
    }
    pendingOAuthFlows.delete(state)
    ...
    try {
      ...
      const tokenResponse = await net.fetch(`${oauthServer}/oauth2/token`, {
        ...
        body: new URLSearchParams({
          ...
          client_id: MARKET_OAUTH_CONFIG.CLIENT_ID,
          code,
          redirect_uri: MARKET_OAUTH_CONFIG.REDIRECT_URI,
      ...
      if (!tokenResponse.ok) {
        ...
        throw new MarketOAuthServiceError(`Failed to exchange code for token: ${tokenResponse.status}`)
      }
      ...
      if (!apiKeysResponse.ok) {
        ...
        throw new MarketOAuthServiceError(`Failed to fetch API keys: ${apiKeysResponse.status}`)
      }
      ...
      if (!apiKeys) {
        throw new MarketOAuthServiceError('No API keys received')
      }
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new MarketOAuthServiceError('Invalid response format from server', error)
      }
      throw error
    }
  }
  ...
  private saveTokenInternal = async (accessToken: string, refreshToken?: string): Promise<void> => {
    ...
    await reduxService.dispatch({
      type: 'llm/setMarketTokens',
      payload
    })
    logger.debug('Successfully saved Market OAuth tokens to Redux')
  }
  ...
  public saveToken = async (
    ...
    try {
      ...
      throw new MarketOAuthServiceError('Failed to save OAuth token', error)
    }
  }
  ...
  public getToken = async (): Promise<string | null> => {
    try {
      const token = await reduxService.select<string>('state.llm.settings.market.accessToken')
      return token || null
    } catch (error) {
  ...
  private getRefreshToken = async (): Promise<string | null> => {
    try {
      const token = await reduxService.select<string>('state.llm.settings.market.refreshToken')
      return token || null
    } catch (error) {
  ...
  private authenticatedFetch = async (
    ...
    const token = await this.getToken()
    if (!token) {
      throw new MarketOAuthServiceError('No OAuth token found')
    }
  ...
  /**
   * Get user balance from Market API
   */
  public getBalance = async (_: Electron.IpcMainInvokeEvent, apiHost: string): Promise<BalanceResponse> => {
    ...
    try {
      ...
      if (!parsed.success) {
        throw new MarketOAuthServiceError('API returned success: false')
      }
      ...
      throw new MarketOAuthServiceError('Failed to get balance', error)
    }
  }
  ...
  public logout = async (_: Electron.IpcMainInvokeEvent, apiHost: string): Promise<void> => {
    ...
    try {
      ...
      await reduxService.dispatch({
        type: 'llm/clearMarketTokens'
      })
      logger.debug('Successfully cleared Market OAuth tokens from Redux')
    } catch (error) {
      logger.error('Failed to logout:', error as Error)
      throw new MarketOAuthServiceError('Failed to logout', error)
    }
  }
}

export default new MarketOAuthService()
