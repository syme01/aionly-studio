...
export const MARKET_PROVIDER_NAME = 'market' as const
export const DEFAULT_MARKET_BASE_URL = 'https://open.cherryin.net/v1'
export const DEFAULT_MARKET_ANTHROPIC_BASE_URL = 'https://open.cherryin.net/v1'
export const DEFAULT_MARKET_GEMINI_BASE_URL = 'https://open.cherryin.net/v1beta/models'
...
export interface MarketProviderSettings {
   ...
   * Market API key.
   *
   * If omitted, the provider will read the `MARKET_API_KEY` environment variable.
   */
  apiKey?: string
  /**
  ...
  fetch?: FetchFunction
  /**
   * Base URL for OpenAI-compatible Market endpoints.
   *
...
export interface MarketProvider extends ProviderV3 {
...
const resolveApiKey = (options: MarketProviderSettings): string =>
  loadApiKey({
    apiKey: options.apiKey,
    environmentVariableName: 'MARKET_API_KEY',
    description: 'Market'
  })
...
class MarketOpenAIChatLanguageModel extends OpenAICompatibleChatLanguageModel {
...
const createJsonHeadersGetter = (options: MarketProviderSettings): (() => Record<string, HeaderValue>) => {
  return () => ({
...
const createAuthHeadersGetter = (options: MarketProviderSettings): (() => Record<string, HeaderValue>) => {
  return () => ({
    Authorization: `Bearer ${resolveApiKey(options)}`,
    ...resolveConfiguredHeaders(options.headers)
  })
...
export const createMarket = (options: MarketProviderSettings = {}): MarketProvider => {
  const {
    baseURL = DEFAULT_MARKET_BASE_URL,
    anthropicBaseURL = DEFAULT_MARKET_ANTHROPIC_BASE_URL,
    geminiBaseURL = DEFAULT_MARKET_GEMINI_BASE_URL,
    fetch,
    endpointType
  } = options
  ...
  const createAnthropicModel = (modelId: string) =>
    new AnthropicMessagesLanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.anthropic`,
  ...
  const createGeminiModel = (modelId: string) =>
    new GoogleGenerativeAILanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.google`,
      ...
      fetch,
      generateId: () => `${MARKET_PROVIDER_NAME}-${Date.now()}`,
      supportedUrls: () => ({})
    })

  const createOpenAIChatModel = (modelId: string, settings: OpenAIProviderSettings = {}) =>
    new MarketOpenAIChatLanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.openai-chat`,
      url,
      headers: () => ({
        ...getJsonHeaders(),
        ...settings.headers
  ...
  const createChatModelByModelId = (modelId: string, settings: OpenAIProviderSettings = {}) => {
    ...
    return new OpenAIResponsesLanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.openai`,
      url,
      headers: () => ({
        ...getJsonHeaders(),
        ...settings.headers
  ...
  const createCompletionModel = (modelId: string, settings: OpenAIProviderSettings = {}) =>
    new OpenAICompletionLanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.completion`,
      url,
      headers: () => ({
        ...getJsonHeaders(),
        ...settings.headers
  ...
  const createEmbeddingModel = (modelId: string, settings: OpenAIProviderSettings = {}) =>
    new OpenAIEmbeddingModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.embeddings`,
      url,
      headers: () => ({
        ...getJsonHeaders(),
        ...settings.headers
  ...
  const createResponsesModel = (modelId: string) =>
    new OpenAIResponsesLanguageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.responses`,
      url,
      headers: () => ({
        ...getJsonHeaders()
      }),
  ...
  const createImageModel = (modelId: string, settings: OpenAIProviderSettings = {}) =>
    new OpenAIImageModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.image`,
      url,
      headers: () => ({
        ...getJsonHeaders(),
        ...settings.headers
  ...
  const createTranscriptionModel = (modelId: string) =>
    new OpenAITranscriptionModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.transcription`,
      url,
      headers: () => ({
        ...getAuthHeaders()
      }),
  ...
  const createSpeechModel = (modelId: string) =>
    new OpenAISpeechModel(modelId, {
      provider: `${MARKET_PROVIDER_NAME}.speech`,
      url,
      headers: () => ({
        ...getJsonHeaders()
      }),
  ...
  return provider as MarketProvider
}
