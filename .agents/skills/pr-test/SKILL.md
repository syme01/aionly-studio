---
name: pr-test
description: Test AiOnly PRs by checking out the branch, launching the Electron app in debug mode, and running interactive UI tests via CDP.
---

# AiOnly PR Test

Automated PR testing workflow for AiOnly. Checks out a PR, launches
...
- Always kill existing AiOnly processes before launching a new instance.
...
- A PR URL (e.g., `https://gitee.com/myme/aionly-studio/pull/13955`)
...
1. If no PR number given, list recent open PRs:
   ```bash
   gh pr list --repo myme/aionly-studio --state open --limit 10 \
...
1. **Kill any existing AiOnly processes** (graceful SIGTERM first):
   ```bash
   pkill -f "aionly.*Electron" 2>/dev/null
...
2. **Start in debug mode** (includes `--remote-debugging-port=9222`):
   ```bash
   nohup pnpm debug > /tmp/aionly-debug.log 2>&1 &
   ```
...
1. **Connect**:
   ...
   ```bash
   WS_URL=$(grep "DevTools listening" /tmp/aionly-debug.log | sed 's/.*ws:/ws:/')
...
2. **Verify connection and identify the main page**:
   ...
   agent-browser tab
   ```
   You should see the main AiOnly page at `http://localhost:5173/`.
...
1. **Kill all AiOnly processes** (graceful SIGTERM first):
   ```bash
   pkill -f "aionly.*Electron" 2>/dev/null
   pkill -f "electron-vite" 2>/dev/null
   lsof -ti :9222 | xargs kill 2>/dev/null
...
If not working:
- Check logs: `tail -50 /tmp/aionly-debug.log`
...
```bash
WS_URL=$(grep "DevTools listening" /tmp/aionly-debug.log | grep 9222 | sed 's/.*\(ws:\/\/[^ ]*\)/\1/')
