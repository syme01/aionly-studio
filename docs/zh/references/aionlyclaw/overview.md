...
<p align="center">
  <img src="./aionlyclaw.png" width="200" alt="AionlyClaw" />
</p>

AionlyClaw 是 AiOnly 中的自主代理（autonomous agent）类型，基于 Claude Agent SDK 构建。与标准的 claude-code 代理不同，AionlyClaw 拥有独立的人格系统、基于任务的调度器、IM 频道集成，以及一组通过内部 MCP 服务器提供的自主管理工具。
...
`SessionMessageService` 不再硬编码 `ClaudeCodeService`，而是通过 `AgentServiceRegistry` 根据 `AgentType` 查找对应的服务实现。AionlyClaw 在运行时通过注册表委托给 claude-code 执行。

```typescript
// src/main/services/agents/services/AgentServiceRegistry.ts
agentServiceRegistry.register('claude-code', new ClaudeCodeService())
agentServiceRegistry.register('aionly-claw', new AionlyClawService())
```

### 自定义系统提示词（替换 Claude Code 预设）

AionlyClaw 不使用 Claude Code 的预设系统提示词。`PromptBuilder` 从工作区文件组装完整的自定义提示词，通过 `_systemPrompt` 字段传递给 `ClaudeCodeService`。当该字段存在时，它作为完整的系统提示词使用，而非预设 + 追加模式。

### 禁用不适用的内置工具

AionlyClaw 通过 `_disallowedTools` 禁用了一组不适合自主运行的 SDK 内置工具：
...
AionlyClaw 采用受 Anna 启发的三文件记忆模型，每个文件有独立的职责范围：

```
{workspace}/
  system.md              — 可选的系统提示词覆盖（替换默认 AionlyClaw 身份）
...
AionlyClaw 使用 Drizzle ORM + LibSQL（SQLite）存储任务数据：
...
| 文件 | 说明 |
|---|---|
| `src/main/services/agents/services/aionlyclaw/index.ts` | AionlyClawService 入口 |
| `src/main/services/agents/services/aionlyclaw/prompt.ts` | PromptBuilder 系统提示词组装 |
| `src/main/services/agents/services/aionlyclaw/heartbeat.ts` | HeartbeatReader 心跳文件读取 |
