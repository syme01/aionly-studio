...
频道系统为 AionlyClaw 提供 IM 集成能力，允许用户通过 Telegram 等即时通讯平台与代理交互。系统采用抽象适配器模式，支持未来扩展到 Discord、Slack 等平台。
...
ChannelManager (单例, 生命周期管理)
  ...
  ├── start()   → 加载所有 AionlyClaw agent，为启用的频道创建适配器
  ├── stop()    → 断开所有适配器
